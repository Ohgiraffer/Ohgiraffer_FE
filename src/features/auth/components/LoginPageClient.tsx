...
export default function LoginPageClient() {
   ...
   const { login, isAuthenticated, isInitializing, bootcampId } = useAuth();
   ...
   // 이미 로그인된 상태로 로그인 페이지에 들어오면 대시보드로 보냄(아직 온보딩 전이면 위저드로) -
   // RequireOnboardingGuard가 결국 같은 곳으로 다시 보내주긴 하지만, 여기서 바로 보내면
   // 대시보드를 거쳤다 다시 튕기는 이중 리다이렉트를 피할 수 있다
   useEffect(() => {
      if (isInitializing || hasCheckedInitialAuthRef.current) return;
      hasCheckedInitialAuthRef.current = true;
      if (isAuthenticated) {
         router.replace(bootcampId === null ? '/onboarding-wizard' : '/');
      }
   }, [isInitializing, isAuthenticated, bootcampId, router]);
