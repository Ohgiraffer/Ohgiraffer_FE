...
export function mapMessageDto(dto: ChatMessageDto, ctx: MapMessageContext): ChatMessage {
   ...
   const senderMember = ctx.members.find((member) => member.userId === dto.senderId);
   const senderName = isMine ? ctx.currentUserName : senderMember?.memberName ?? '참여자';
   const sentAtDate = new Date(dto.sentAt);
   return {
      id: dto.sendbirdMessageId,
      senderId: dto.senderId,
      senderName,
      senderProfileImageUrl: senderMember?.profileImageUrl ?? null,
      ...
      content: dto.content ?? '',
      // 실제 응답에서 첨부파일 없는 메시지의 attachmentUrl이 JSON null이 아니라 문자열 "null"로
      // 오는 경우가 확인돼(검색 API 실제 응답 예시), 그 값도 첨부파일 없음으로 취급한다
      attachmentUrl: dto.attachmentUrl === 'null' ? null : dto.attachmentUrl,
      sentAt: isValid(sentAtDate) ? format(sentAtDate, 'HH:mm') : '',
      sentAtISO: dto.sentAt,
      isMine,
      // 재배포 전 응답에는 아직 없을 수 있어 기본값 0으로 방어
